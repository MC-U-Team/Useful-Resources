...
public class BasicTrapDoorBlock extends TrapDoorBlock implements IBlockItemProvider, IParticleBlock, IBlockRenderType {
	
	protected final BlockItem blockItem;
	
	public BasicTrapDoorBlock(Rarity rarity, int harvestLevel, float hardness, float resistance) {
		super(Properties.create(Material.IRON).sound(SoundType.METAL).harvestTool(ToolType.PICKAXE).harvestLevel(harvestLevel).setRequiresTool().hardnessAndResistance(hardness, resistance).notSolid().setAllowsSpawn((state, world, pos, type) -> false));
