...
public class ResourceRecipesProvider extends CommonProvider {
	...
	private void registerResourceRecipes(IResource resource, Consumer<IFinishedRecipe> consumer) {
		...
		ShapelessRecipeBuilder.shapelessRecipe(items.getItem(NUGGET), 9).addIngredient(items.getTag(INGOT)).addCriterion("has_" + name + "_nugget", hasItem(items.getTag(NUGGET))).build(consumer, createLocation(resource, "crafting/nugget_from_ingot"));
