...
public class ResourceBlockStatesProvider extends CommonBlockStatesProvider {
	...
	protected void resourceTypeBlock(IResource resource, IResourceType<Block> type, Block block, ResourceLocation baseModel, IDataGeneratorConfigurator dataGeneratorConfigurator) {
		...
		if (extraProperties.containsKey(type.getName() + "ModelOverride")) {
			...
			final IResourceDataType resourceDataType;
			if (ObjectUtil.getBoolean(extraProperties.getOrDefault("ingotModel", false))) {
				resourceDataType = ResourceDataType.INGOT;
			} else {
				resourceDataType = dataGeneratorConfigurator.getResourceType();
			}
			
			if (type == BlockResourceType.ORE) {
				model = modLoc("base/block/special/" + resourceDataType.getName() + "_stone_ore");
			} else if (type == BlockResourceType.NETHER_ORE) {
				model = modLoc("base/block/special/" + resourceDataType.getName() + "_netherrack_nether_ore");
			} else if (type == BlockResourceType.END_ORE) {
				model = modLoc("base/block/special/" + resourceDataType.getName() + "_endstone_end_ore");
			} else if (type == BlockResourceType.BLOCK) {
				model = modLoc("base/block/special/" + resourceDataType.getName() + "_" + type.getName());
			} else {
				model = null;
			}
		}
